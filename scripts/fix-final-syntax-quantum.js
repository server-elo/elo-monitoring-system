...
const fixes = [
  ...
  {
    pattern: /^(\s*)(private|public|protected)?\s*(readonly)?\s*(\w+):\s*(.+);$/gm,
    replacement: (match, indent, visibility = '', readonly = '', name, value) => {
      // Skip type declarations
      if (value.includes('===>') || value.includes('(') || value.match(/^[A-Z]/)) {
        return match;
      }
      return `${indent}${visibility} ${readonly} ${name} = ${value};`.replace(/\s+/g, ' ').trim() + ';';
    },
  ...
  {
    pattern: /className="text-blue-400,\s*hover:text-blue-300"/g,
    replacement: 'className="text-blue-400 hover:text-blue-300"',
  ...
  {
    pattern: /const event: AnalyticsEvent:\s*{/g,
    replacement: 'const event: AnalyticsEvent = {',
    description: 'Fix AnalyticsEvent assignment'
  },
  
  // Fix trackEvent function
  {
    pattern: /trackEvent\(\s*name: string/g,
    replacement: 'trackEvent(name: string',
    description: 'Fix trackEvent function'
  },
  
  // Fix sendToGA function
  {
    pattern: /this\.sendToGA\(\s*name,/g,
    replacement: 'this.sendToGA(name,',
    description: 'Fix sendToGA call'
  },
  
  // Fix logger.info call
  {
    pattern: /logger\.info\(`Analytics,\s*event:/g,
    replacement: 'logger.info(`Analytics event:',
    description: 'Fix logger.info call'
  },
  ...
  // Fix closing braces
  {
    pattern: /}\)\);\s*}\)\);/g,
    replacement: '  });\n  }',
    description: 'Fix closing braces'
  },
  
  // Fix timestamp property in analytics
  {
    pattern: /(\s+)timestamp: Date\.now\(\)\s*$/gm,
    replacement: '$1timestamp: Date.now()',
    description: 'Fix timestamp assignment'
  },
  
  // Fix performanceMetric assignment
  {
    pattern: /const performanceMetric: PerformanceMetrics:\s*{/g,
    replacement: 'const performanceMetric: PerformanceMetrics = {',
    description: 'Fix performanceMetric assignment'
  },
  
  // Fix reduce function calls
  {
    pattern: /\.reduce\(\s*\(acc/g,
    replacement: '.reduce((acc',
    description: 'Fix reduce syntax'
  },
  
  // Fix map function issue
  {
    pattern: /\.map\(e\)\s*=>/g,
    replacement: '.map(e =>',
  ...
  // Fix flushTimestamp assignment
  {
    ...
    replacement: 'flushTimestamp: Date.now()',
    description: 'Fix flushTimestamp assignment'
  },
  
  // Fix template literal syntax
  {
    pattern: /console\.log\(_`([^`]+)`\)/g,
  ...
  {
    pattern: /\.sort\(\s*\(\s*\[\s*a\s*\],\s*\[\s*b\s*\]\s*\)\s*=>\s*b\s*-\s*a\s*\)/g,
    replacement: '.sort(([, a], [, b]) => b - a)',
    description: 'Fix array sort syntax'
  },
  
  // Fix AchievementGrid specific issues
  {
    pattern: /achievements:\s*userAchievements,/g,
    replacement: 'achievements, userAchievements,',
    description: 'Fix AchievementGrid props'
  },
  
  // Fix motion.div key prop
  {
    pattern: /<motion\.div,\s*key:\s*{([^}]+)}/g,
    replacement: '<motion.div key={$1}',
    description: 'Fix motion.div key prop'
  },
  
  // Fix initial prop syntax
  {
    pattern: /initial:\s*{{\s*([^}]+)\s*}}/g,
    replacement: 'initial={{ $1 }}',
    description: 'Fix initial prop syntax'
  },
  
  // Fix performance Record type
  {
    pattern: /performance: Record<string\s*{\s*avg:/g,
    replacement: 'performance: Record<string, { avg:',
    description: 'Fix performance Record type'
  },
  
  // Fix Record string syntax
  {
    pattern: /Record<string\s*{/g,
    replacement: 'Record<string, {',
    description: 'Fix Record generic syntax'
  }
];
