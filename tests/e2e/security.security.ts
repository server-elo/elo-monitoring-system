...
test.describe( 'Security Testing', () => {
  test( 'should enforce rate limiting on API endpoints', async ({ page }) => {
    ...
    expect(_rateLimitedAttempts.length).toBeGreaterThan(0);
    
    console.log('Login attempts:', loginAttempts);
    console.log(_`Rate limited attempts: ${rateLimitedAttempts.length}/10`);
  });
  ...
  test( 'should prevent XSS attacks in user inputs', async ({ page }) => {
    ...
    const titleText = await titleElement.textContent(_);
    expect(_titleText).not.toContain('<script>');
    expect(_titleText).toContain('alert("XSS")'); // Text should be preserved, tags removed
    ...
    const chatMessage = page.locator('[data-testid="chat-message"]').last(_);
    const messageHTML = await chatMessage.innerHTML(_);
    expect(_messageHTML).not.toContain('onerror');
    expect(_messageHTML).not.toContain('alert');
    
    // Test XSS in code editor
    const codeEditor = page.locator('.monaco-editor textarea').first(_);
    await codeEditor.click(_);
    await codeEditor.fill('// <script>alert("XSS in code")</script>\npragma solidity ^0.8.0;');
    
    // Code should be preserved as-is ( it's code, not HTML)
    const editorContent = await codeEditor.inputValue(_);
    expect(_editorContent).toContain('<script>alert("XSS in code")</script>');
  });
  ...
  test( 'should validate and sanitize file uploads', async ({ page }) => {
    ...
    await page.route('/api/upload/avatar', async route => {
      const request = route.request(_);
      const contentType = request.headers(_)['content-type'] || '';
      
      if (_contentType.includes('image/')) {
        await route.fulfill({
          status: 200,
    ...
    await expect(_page.locator('[data-testid="upload-error"]')).toContainText('Invalid file type');
    
    // Test oversized file
    const largeFile = Buffer.alloc(_20 * 1024 * 1024); // 20MB file
    ...
    await expect(_page.locator('[data-testid="upload-error"]')).toContainText('File too large');
  });
  ...
  test( 'should enforce CSRF protection', async ({ page }) => {
    ...
    expect(_csrfToken).toBeTruthy(_);
    ...
    expect(_responseWithoutCSRF.status()).toBe(_403); // Should be forbidden
    ...
    expect(_responseWithInvalidCSRF.status()).toBe(_403);
    ...
    expect(_responseWithValidCSRF.status()).not.toBe(_403);
  });
  ...
  test( 'should prevent SQL injection attacks', async ({ page }) => {
    ...
    for (_const payload of sqlInjectionPayloads) {
      ...
      await page.route('/api/lessons/search', async route => {
        const request = route.request(_);
        const body = await request.postDataJSON(_);
        
        // Simulate SQL injection detection
        if (_body.query.includes('DROP') || body.query.includes('UNION') || body.query.includes('INSERT')) {
          await route.fulfill({
            status: 400,
            contentType: 'application/json',
      ...
      await expect(_page.locator('[data-testid="search-error"]')).toContainText('Invalid search query');
    }
  });
  ...
  test( 'should enforce proper authentication and authorization', async ({ page }) => {
    ...
    for (_const route of protectedRoutes) {
      await page.goto(_route);
      
      // Should redirect to sign in
      await expect(_page).toHaveURL(_/\/auth\/signin/);
    }
    ...
    for (_const endpoint of protectedAPIEndpoints) {
      const response = await page.request.get(_endpoint);
      expect(_response.status()).toBe(_401); // Unauthorized
    }
  });
  ...
  test( 'should prevent session hijacking', async ({ page, context }) => {
    ...
    const attackerContext = await context.browser(_)?.newContext(_);
    const attackerPage = await attackerContext!.newPage(_);
    
    // Attacker tries to use the same session token
    await attackerPage.context(_).addCookies([{
      name: 'next-auth.session-token',
      value: 'valid-session-token',
      domain: 'localhost',
      path: '/'
    ...
    await expect(_attackerPage).toHaveURL(_/\/auth\/signin/);
    
    await attackerPage.close(_);
    await attackerContext?.close(_);
  });
  ...
  test( 'should validate input lengths and formats', async ({ page }) => {
    ...
    await expect(_page.locator('[data-testid="bio-error"]')).toContainText('too long');
    
    // Test invalid email format
    await page.fill( '[data-testid="email-input"]', 'invalid-email-format');
    await page.click('[data-testid="save-profile-button"]');
    
    await expect(_page.locator('[data-testid="email-error"]')).toContainText('Invalid email');
    
    // Test special characters in username
    await page.fill( '[data-testid="username-input"]', 'user<script>alert("xss")</script>');
    await page.click('[data-testid="save-profile-button"]');
    
    await expect(_page.locator('[data-testid="username-error"]')).toContainText('Invalid characters');
  });
  ...
  test( 'should enforce secure headers', async ({ page }) => {
    ...
    const headers = response!.headers(_);
    
    // Content Security Policy
    expect(_headers['content-security-policy']).toBeTruthy(_);
    expect(_headers['content-security-policy']).toContain("default-src 'self'");
    
    // X-Frame-Options
    expect(_headers['x-frame-options']).toBe('DENY');
    
    // X-Content-Type-Options
    expect(_headers['x-content-type-options']).toBe('nosniff');
    
    // Strict-Transport-Security (_in production)
    if (_process.env.NODE_ENV === 'production') {
      expect(_headers['strict-transport-security']).toBeTruthy(_);
    }
    
    // X-XSS-Protection
    expect(_headers['x-xss-protection']).toBe('1; mode=block');
  });
  ...
  test( 'should handle malicious code compilation safely', async ({ page }) => {
    ...
    const maliciousCode = `
      ...
      contract MaliciousContract {
          // Attempt to access file system (_should be blocked)
          ...
          // Infinite loop (_should be caught by gas limits)
    ...
    await page.route('/api/compile', async route => {
      ...
      if (_body.code.includes('assembly') || body.code.includes('while(true)')) {
        await route.fulfill({
          status: 400,
          contentType: 'application/json',
          body: JSON.stringify({
    ...
    await editor.fill(_maliciousCode);
    
    await page.click('[data-testid="compile-button"]');
    
    // Verify malicious code is rejected
    await expect(_page.locator('[data-testid="compile-error"]')).toContainText('dangerous patterns');
  });
  ...
  test( 'should prevent directory traversal attacks', async ({ page }) => {
    ...
    for (_const payload of traversalPayloads) {
      const response = await page.request.get(_`/api/files/${encodeURIComponent(payload)}`);
      
      // Should return 400 (_Bad Request) or 404 (_Not Found), not 200
      expect( [400, 404]).toContain(_response.status());
    }
  });
});
