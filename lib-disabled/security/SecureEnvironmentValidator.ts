...
const createSecureEnvSchema = (environment: string) => {
  ...
  return z.object({
          ...
          if (isProd) {
            ...
            for (const pattern of weakPatterns) {
              if (credentials.toLowerCase().includes(pattern)) {
                throw new Error(
                  `Weak database credentials detected: contains "${pattern}"`,
                )
              }
            }
          }
    ...
    NEXTAUTH_SECRET: z
      ...
      .min(
        isProd ? 64 : 32,
        `NEXTAUTH_SECRET must be at least ${isProd ? 64 : 32} characters`,
      )
      .refine((secret) => {
        const validation = quantumSecrets.validateSecretStrength(secret, 'auth')
        if (!validation.valid) {
          throw new Error(
            `NEXTAUTH_SECRET validation failed: ${validation.issues.join(', ')}`,
          )
        }
        return true
      }),
    ...
    JWT_SECRET: z
      ...
      .min(
        isProd ? 64 : 32,
        `JWT_SECRET must be at least ${isProd ? 64 : 32} characters`,
      )
      .refine((secret) => {
        const validation = quantumSecrets.validateSecretStrength(secret, 'auth')
        if (!validation.valid) {
          throw new Error(
            `JWT_SECRET validation failed: ${validation.issues.join(', ')}`,
          )
        }
        return true
      })
    ...
    SESSION_SECRET: z
      ...
      .min(
        isProd ? 64 : 32,
        `SESSION_SECRET must be at least ${isProd ? 64 : 32} characters`,
      )
      .refine((secret) => {
        const validation = quantumSecrets.validateSecretStrength(secret, 'auth')
        if (!validation.valid) {
          throw new Error(
            `SESSION_SECRET validation failed: ${validation.issues.join(', ')}`,
          )
        }
        return true
      })
    ...
    GITHUB_WEBHOOK_SECRET: z
      ...
      .refine((secret) => {
        ...
        const validation = quantumSecrets.validateSecretStrength(
          secret,
          'webhook',
        )
        if (!validation.valid) {
          throw new Error(
            `GITHUB_WEBHOOK_SECRET validation failed: ${validation.issues.join(', ')}`,
          )
        }
        return true
      })
    ...
    INFURA_PROJECT_SECRET: z
      .string()
      .min(32, 'Infura project secret too short')
      .optional(),

    // Cloud services
    AWS_ACCESS_KEY_ID: z
      .string()
      .length(20, 'AWS Access Key ID must be 20 characters')
      .optional(),
    AWS_SECRET_ACCESS_KEY: z
      .string()
      .min(40, 'AWS Secret Access Key too short')
      .optional(),
    ...
    SENTRY_AUTH_TOKEN: z
      .string()
      .min(64, 'Sentry auth token too short')
      .optional(),

    // Analytics
    NEXT_PUBLIC_MIXPANEL_TOKEN: z
      .string()
      .min(32, 'Mixpanel token too short')
      .optional(),
  ...
  private performSecurityAudit(
    ...
    if (isProd) {
      ...
      for (const key of requiredProdSecrets) {
        if (!envVars[key]) {
          result.criticalIssues.push(
            `${key} is required in production environment`,
          )
        }
      }
    }
      ...
      for (const key of urlKeys) {
        ...
        if (value) {
          if (value.includes('localhost') || value.includes('127.0.0.1')) {
            result.criticalIssues.push(
              `${key} cannot use localhost in production`,
            )
          }
    ...
    const exposurePatterns = [
      {
        pattern: /^(test|dev|staging|example|demo|placeholder)$/i,
        message: 'Appears to be a placeholder value',
      },
      {
        pattern: /^(password|admin|user|guest|root)$/i,
        message: 'Uses common weak pattern',
      },
      { pattern: /^.{1,8}$/, message: 'Too short for secure use' },
      { pattern: /^(.)\1{3,}$/, message: 'Contains repeating characters' },
    ]

    const secretKeys = Object.keys(envVars).filter(
      (key) =>
        key.includes('SECRET') ||
        key.includes('KEY') ||
        key.includes('TOKEN') ||
    ...
    if (envVars.QUANTUM_ROTATION_ID && envVars.QUANTUM_ROTATION_ENV) {
      result.info.push(
        `✅ Quantum secrets rotation detected: ${envVars.QUANTUM_ROTATION_ID}`,
      )
      ...
      result.warnings.push(
        'No quantum rotation metadata found - consider running secrets rotation',
      )
    }
  }
...
export function requireSecureEnvironment(): Record<string, unknown> {
  ...
  if (result.criticalIssues.length > 0) {
    ...
    throw new Error(
      'Environment validation failed due to critical security issues',
    )
  }
