...
export class SolidityDebugger extends EventEmitter {
  ...
  removeBreakpoint(sessionId: string, breakpointId: string): boolean {
    ...
    for (const [line, breakpoint] of session.breakpoints.entries()) {
  ...
  /**
   * Get a breakpoint by session and breakpoint ID
   */
  getBreakpoint(sessionId: string, breakpointId: string): BreakpointInfo | null {
    const session = this.sessions.get(sessionId);
    if (!session) {
      return null;
    }
  ...
  async continue(sessionId: string): Promise<void> {
    ...
    try {
      ...
      await this.executeContinue(session);
      
      this.emit('execution-continued', { sessionId });
    } catch (error) {
  ...
  async getVariableValue(
    ...
    const targetFrame = frameIndex ?? session.state.currentFrame;
    
    if (targetFrame < 0 || targetFrame >= session.state.callStack.length) {
      return null;
    }

    const frame = session.state.callStack[targetFrame];
  ...
  private async executeStep(
    ...
    if (breakpoint && breakpoint.enabled) {
      ...
      this.emit('breakpoint-hit', { breakpoint });
    }
  ...
  private async executeContinue(session: DebugSession): Promise<void> {
