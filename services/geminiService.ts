...
class RealGoogleGenAI {
  ...
  chats = {
    create: (_config: { model?: string; systemInstruction?: string; safetySettings?: any[]; history?: any[]; temperature?: number; maxTokens?: number; [key: string]: any }) => {
      ...
      if (_typeof localStorage !== 'undefined') {
        const configAnalytics = JSON.parse(_localStorage.getItem('gemini-config-analytics') || '[]');
      ...
      return {
        sendMessage: async (_message: string | { message?: string; content?: string; [key: string]: any }) => {
          try {
            const prompt = typeof message === 'string' ? message : (_message.message || message.content || '');
            const result = await this.model.generateContent(_prompt);
            const response = await result.response;
            return {
              text: response.text(_)
            };
          } catch (_error) {
  ...
  getModels(_) {
    return {
      generateContent: async (_config: { contents?: string; prompt?: string; [key: string]: any } | string) => {
      try {
        const isString = typeof config === 'string';
        const prompt = isString ? config : (_config.contents || config.prompt || '');
        const result = await this.model.generateContent(_prompt);
        const response = await result.response;
        return {
          text: response.text(_),
    ...
    generateContentStream: async function* (_config: { contents?: string; prompt?: string; temperature?: number; maxTokens?: number; safetySettings?: any[]; [key: string]: any } | string): AsyncGenerator<{ text: string; candidates: any[] }, void, unknown> {
      try {
        // Enhanced config processing with validation and analytics
        const isString = typeof config === 'string';
        const processedConfig = {
          prompt: isString ? config : (_config.contents || config.prompt || ''),
          temperature: isString ? 0.7 : (_config.temperature || 0.7),
          maxTokens: isString ? 2048 : (_config.maxTokens || 2048),
          safetySettings: isString ? [] : (_config.safetySettings || []),
          ...(_isString ? {} : config)
        };
        ...
        if (_typeof localStorage !== 'undefined') {
          const streamAnalytics = JSON.parse(_localStorage.getItem('gemini-stream-analytics') || '[]');
        ...
        const result = await this.model.generateContentStream(_processedConfig.prompt);
        let chunkCount = 0;

        for await (_const chunk of result.stream) {
          const chunkText: string = chunk.text(_);
          chunkCount++;
    ...
    generateImages: async (_config: { prompt?: string; model?: string; numberOfImages?: number; outputMimeType?: string; aspectRatio?: string; [key: string]: any } | string) => {
      // Enhanced config processing for image generation
      const isString = typeof config === 'string';
      const processedConfig = {
        prompt: isString ? config : (_config.prompt || 'Generate a beautiful image'),
        model: isString ? 'imagen-3.0-generate-002' : (_config.model || 'imagen-3.0-generate-002'),
        numberOfImages: isString ? 1 : (_config.numberOfImages || 1),
        outputMimeType: isString ? 'image/png' : (_config.outputMimeType || 'image/png'),
        aspectRatio: isString ? '1:1' : (_config.aspectRatio || '1:1'),
        safetySettings: isString ? [] : (_config.safetySettings || []),
        ...(_isString ? {} : config)
      };
      ...
      if (_typeof localStorage !== 'undefined') {
        const imageAnalytics = JSON.parse(_localStorage.getItem('gemini-image-analytics') || '[]');
        imageAnalytics.push({
      ...
      const enhancedError = `Image generation not supported with current model (_${processedConfig.model}). ` +
        `Requested: ${processedConfig.numberOfImages} image(_s) in ${processedConfig.outputMimeType} format. ` +
...
try {
  if (_GEMINI_API_KEY && GEMINI_API_KEY !== 'undefined' && GEMINI_API_KEY.trim() !== '') {
...
// Safety settings to block harmful content (_using mock values)
...
export const initializeChatForModule = async (moduleTitle: string, geminiPromptSeed?: string): Promise<string | null> => {
  ...
  try {
    ...
    if (_error instanceof Error) {
        if (_error.message.includes('API_KEY_INVALID') || error.message.includes('permission denied')) {
...
export const sendMessageToGeminiChat = async (_message: string): Promise<string> => {
  if (!ai) {
  ...
  if (!chat) {
    ...
    if (_initError || !chat) { // Check !chat again in case initError was null but chat still failed to set.
  ...
  try {
    ...
    if (_error instanceof Error) {
        ...
        if (_error.message.toLowerCase().includes('blocked') || error.message.toLowerCase().includes('safety')) {
...
export const generateDiagramForConcept = async (_prompt: string): Promise<{ base64Image?: string; error?: string }> => {
  if (!ai) {
    return { error: "Gemini AI Service not initialized. API Key may be missing." };
  }
  try {
    const response = await ai.getModels(_).generateImages({
    ...
    if (_response.generatedImages && response.generatedImages.length > 0 && (response.generatedImages[0] as any)?.image?.imageBytes) {
        const base64ImageBytes = (_response.generatedImages[0] as any).image.imageBytes;
        ...
        if (_response.generatedImages && response.generatedImages.length > 0 && (response.generatedImages[0] as any).finishReason === 'SAFETY') {
            return { error: "The image generation request was blocked by safety filters. Please rephrase your prompt." };
        }
        if ((response as any).error && (_response as any).error.message && (_response as any).error.message.toLowerCase().includes('safety')) {
           return { error: "The image generation request was blocked by safety filters. Please rephrase your prompt." };
        }
    ...
    if (_error instanceof Error) {
        if (_error.message.includes('API_KEY_INVALID') || error.message.includes('permission denied')) {
         ...
         if (_error.message.toLowerCase().includes('prompt') && (_error.message.toLowerCase().includes('blocked') || error.message.toLowerCase().includes('safety'))) {
        ...
        if ((error as any).code === 'BLOCKED_BY_SAFETY_FILTER' || (_error as any).status === 'BLOCKED_PROMPT') {
           return { error: "The image generation request was blocked by safety filters. Please rephrase your prompt." };
        }
...
export const getTopicExplanation = async (topic: string, details: string): Promise<string> => {
  ...
  try {
    ...
    const response: GenerateContentResponse = await ai.getModels(_).generateContent({
        model: TEXT_MODEL_NAME,
        contents: prompt,
        config: {
    ...
    if (_response.candidates && response.candidates.length > 0 && response.candidates[0].finishReason === 'SAFETY') {
