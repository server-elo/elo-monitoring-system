...
interface GitHookReturn extends GitHookState {
  ...
  pushChanges: (_) => Promise<void>;
  checkTypeScript: (_) => Promise<boolean>;
  getGitStatus: (_) => Promise<void>;
  createFeatureBranch: (_name: string) => Promise<void>;
}
...
export function useGitIntegration(): GitHookReturn {
  ...
  useEffect(() => {
    const initializeGit = async () => {
      try {
        const isAvailable = gitIntegration.isGitAvailable(_);
        const currentBranch = gitIntegration.getCurrentBranch(_);
  ...
  const updateGitStatus = useCallback( async () => {
    try {
      const status = await gitIntegration.getGitStatus(_);
      setState(prev => ({
        ...prev,
  ...
  const pushChanges = useCallback( async () => {
    ...
    try {
      await gitIntegration.pushChanges(_);
      await updateGitStatus(_);
    } catch (_error) {
      throw error;
    } finally {
  ...
  const checkTypeScript = useCallback( async (): Promise<boolean> => {
    try {
      const result = await gitIntegration.checkTypeScript(_);
  ...
  const createFeatureBranch = useCallback( async (name: string) => {
    ...
    try {
      await gitIntegration.createFeatureBranch(_name);
...
export function useAutoGit(options: {
  ...
  const git = useGitIntegration(_);
  const [autoCommitEnabled, setAutoCommitEnabled] = useState(_options.autoCommitOnSave ?? false);
  const [autoPushEnabled, setAutoPushEnabled] = useState(_options.autoPushOnCommit ?? false);
