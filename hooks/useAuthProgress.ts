...
export function useAuthProgress(): void {
  ...
  // Update progress
  const updateProgress = useCallback(async (data: ProgressData) => {
    if (!session?.user?.id) return;
    try {
...
const startLesson = useCallback(async (lessonId: string, moduleId?: string, courseId?: string) => {
...
const getProgress = useCallback((params: {
  courseId?: string;
  moduleId?: string;
  lessonId?: string;
}) => {
