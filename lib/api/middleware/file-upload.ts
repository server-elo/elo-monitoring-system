/**
 * File upload middleware for Next.js API routes
 */
...
export function withFileUpload(
  handler: ( req: NextRequest, files: any[], fields: Record<string, string>) => Promise<Response>,
  ...
  return async (_request: NextRequest) => {
    ...
    try {
      // Parse form data
      const { files, fields } = await parseFormData(_request);
      ...
      if (_files.length > maxFiles) {
        return NextResponse.json(
          { error: `Maximum ${maxFiles} file(_s) allowed` },
          { status: 400 }
        );
      }
      ...
      for (_const file of files) {
        ...
        validatedFiles.push(_file);
      }

      // Call the handler with validated files
      return handler( request, validatedFiles, fields);
