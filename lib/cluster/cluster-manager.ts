/**
 * Cluster Manager for Perfect 12-Factor Compliance
 * Implements process-based concurrency scaling (Factor VIII)
 */
...
class ClusterManager {
  ...
  constructor(config: ClusterConfig = {}) {
    this.config = {
      ...
      enabledInDevelopment: config.enabledInDevelopment || false,
    };
  }

  /**
   * Start cluster or single process based on environment
   */
  start(workerCallback: () => void): void {
    const shouldCluster = this.shouldEnableCluster();
    ...
    if (cluster.isPrimary) {
      ...
      this.startWorker(workerCallback);
    }
  }
   ...
   */
  private shouldEnableCluster(): boolean {
    const nodeEnv = process.env.NODE_ENV;
    
    // Always enable in production
    if (nodeEnv === 'production') return true;
    
    // Enable in development only if explicitly configured
    if (nodeEnv === 'development') return this.config.enabledInDevelopment;
  ...
  /**
   * Start primary process (cluster master)
   */
  private startPrimary(): void {
    logger.info('Starting cluster primary process', {
    ...
    cluster.on('online', (worker) => {
      logger.info('Worker came online', { 
        workerId: worker.id, 
    ...
    logger.info('Cluster primary process started successfully');
  }

  /**
   * Start worker process
   */
  private startWorker(callback: () => void): void {
    ...
    gracefulShutdown.register({
      ...
      handler: async () => {
        ...
        // Worker-specific cleanup can be added here
      },
      timeout: 10000,
    });
  ...
  private handleWorkerExit(worker: any, code: number, signal: string): void {
    if (this.isShuttingDown) {
      logger.info('Worker exited during shutdown', { 
        workerId: worker.id, 
        code, 
        signal 
      });
    ...
    const shutdownPromises = workers.map(worker => {
      ...
      return new Promise<void>((resolve) => {
        const timeout = setTimeout(() => {
          logger.warn('Force killing worker due to timeout', { 
            workerId: worker.id 
          });
          worker.kill();
          resolve();
...
export const clusterManager = new ClusterManager({
  workers: parseInt(process.env.CLUSTER_WORKERS || '0') || cpus().length,
  maxRestarts: parseInt(process.env.CLUSTER_MAX_RESTARTS || '5'),
  restartDelay: parseInt(process.env.CLUSTER_RESTART_DELAY || '1000'),
  enabledInDevelopment: process.env.CLUSTER_DEV_ENABLED === 'true',
});
