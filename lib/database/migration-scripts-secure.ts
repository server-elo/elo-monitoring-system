...
const ALLOWED_TABLES = [
  'User', 'Course', 'Lesson', 'UserProgress', 'Achievement', 
  'UserAchievement', 'CourseEnrollment', 'CollaborationSession', 
  'ChatMessage'
] as const;
...
export class SecureDatabaseMigrationManager {
  ...
  private validateTableName(tableName: string): asserts tableName is AllowedTable {
    if (!ALLOWED_TABLES.includes(tableName as AllowedTable)) {
      throw new Error(`Invalid table name: ${tableName}. Only allowed tables: ${ALLOWED_TABLES.join(', ')}`);
    }
  }
  ...
  private validateNumericParams(offset: number, limit: number): void {
    if (!Number.isInteger(offset) || !Number.isInteger(limit) || offset < 0 || limit < 1 || limit > 10000) {
      throw new Error(`Invalid numeric parameters. Offset: ${offset}, Limit: ${limit}`);
    }
  }
  ...
  private async migrateTable(
    ...
    try {
      ...
      errors.push(`Table ${plan.tableName}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      return {
        success: false,
        migratedCount,
        errors,
  ...
  private async updateSequences(): Promise<void> {
    ...
    for (const { sequence, table } of sequenceUpdates) {
      try {
        ...
        this.log(`Warning: Failed to update sequence ${sequence}: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    }
  }
  ...
  private async validateSourceDatabase(): Promise<void> {
    try {
      ...
      throw new Error(`Cannot connect to SQLite database: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
  ...
  private async validateTargetDatabase(): Promise<void> {
    try {
      ...
      throw new Error(`Cannot connect to PostgreSQL database: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
