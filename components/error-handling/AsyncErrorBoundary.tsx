  ...
  private getErrorMessage = () => {
    ...
    switch (operationType) {
      ...
      return {
        title: "Upload Failed",
        description:
        "Your file upload was interrupted. Please try uploading again.",
        icon: AlertTriangle,
        color: "text-red-500"
      };
      case ",
  ...
  render() {
    if (this.state.hasError) {
      ...
      return (
        ...
        transition={{ duration: 0.2 }}><GlassContainer
        intensity="medium"
        className="p-6 text-center max-w-md mx-auto"><div className="mb-4">
        <div
        className={`w-12 h-12 mx-auto bg-red-500/20 rounded-full flex items-center justify-center mb-3`}><Icon className={`w-6 h-6 ${errorMessage.color}`} />
        </div>
        ...
        {retryCount < maxRetries && (
          ...
          disabled={
            isRetrying ||
            (!isOnline && this.props.operationType !== "processing")
          }
          className="flex items-center space-x-2 px-4 py-2 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-800 disabled:cursor-not-allowed text-white rounded-lg transition-colors text-sm"><RefreshCw
        ...
        {retryCount >= maxRetries && (
          ...
          className="flex items-center space-x-2 px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white rounded-lg transition-colors text-sm"><RefreshCw className="w-4 h-4" />
          <span>Reset</span>
          </button>
        )}
        </div>
