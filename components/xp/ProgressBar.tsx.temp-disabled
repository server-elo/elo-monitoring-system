...
export function ProgressBar({
  ...
  const getSizeClasses = () => {
    switch (size) {
      case 'sm':
        return { container: 'h-2', text: 'text-xs' }
      case 'md':
        return { container: 'h-3', text: 'text-sm' }
      case 'lg':
        return { container: 'h-4', text: 'text-base' }
      default:
        return { container: 'h-3', text: 'text-sm' }
    }
  }
        ...
        <div className="flex justify-between items-center mb-2">
          ...
          <div className="flex items-center space-x-2">
            {showValues &&
              ...
              maxValue !== undefined && (
                <span
                  className={cn('font-mono text-gray-400', sizeClasses.text)}
                >
...
export function LevelProgressBar({
  ...
  previousXP,
  className,
}: LevelProgressBarProps): ReactElement {
  const xpInCurrentLevel = currentXP - xpForCurrentLevel
  const xpNeededForNextLevel = xpForNextLevel - xpForCurrentLevel
  const progressPercentage = (xpInCurrentLevel / xpNeededForNextLevel) * 100

  const previousXPInLevel = previousXP
    ? Math.max(0, previousXP - xpForCurrentLevel)
    : undefined
  const previousProgressPercentage = previousXPInLevel
    ? (previousXPInLevel / xpNeededForNextLevel) * 100
    : undefined
