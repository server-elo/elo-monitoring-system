...
export function LevelUpCelebration({
  ...
  autoClose = false,
  autoCloseDelay = 8000,
}: LevelUpCelebrationProps): ReactElement {
  const [showRewards, setShowRewards] = useState(false)
  const [celebrationPhase, setCelebrationPhase] = useState<
    'entrance' | 'celebration' | 'rewards'
  >('entrance')
              ...
              {showRewards && levelUpData.rewards && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {/* Badges */}
                    {levelUpData.rewards.badges &&
                      levelUpData.rewards.badges.length > 0 && (
                        <motion.div
                          initial={{ scale: 0.8, opacity: 0 }}
                          animate={{ scale: 1, opacity: 1 }}
                          transition={{ delay: 0.6 }}
                        ...
                        >
                          ...
                          <div className="text-lg font-bold text-purple-300">
                            {levelUpData.rewards.badges.length} New Badge
                            {levelUpData.rewards.badges.length > 1 ? 's' : ''}
                          </div>
                          <div className="text-sm text-gray-400">
                    ...
                    {/* Titles */}
                    {levelUpData.rewards.titles &&
                      levelUpData.rewards.titles.length > 0 && (
                        <motion.div
                          initial={{ scale: 0.8, opacity: 0 }}
                          animate={{ scale: 1, opacity: 1 }}
                        ...
                        >
                          ...
                          <div className="text-lg font-bold text-blue-300">
                            New Title
                            {levelUpData.rewards.titles.length > 1 ? 's' : ''}
                          </div>
                          <div className="text-sm text-gray-400">
                    ...
                    {/* Unlocks */}
                    {levelUpData.rewards.unlocks &&
                      levelUpData.rewards.unlocks.length > 0 && (
                        <motion.div
                          initial={{ scale: 0.8, opacity: 0 }}
                          animate={{ scale: 1, opacity: 1 }}
                          transition={{ delay: 0.8 }}
...
export function LevelUpManager({
  className,
}: LevelUpManagerProps): ReactElement {
  ...
  return (
    <div className={className}>
      {levelUpData && (
        <LevelUpCelebration levelUpData={levelUpData} onClose={closeLevelUp} />
      )}
    </div>
  )
}
