...
export function VersionControlInterface({
  ...
  className,
}: VersionControlInterfaceProps): ReactElement {
  const {
    isInitialized,
    currentBranch,
    branches,
    ...
    getFileDiff,
    openMergeRequests,
  } = useSolidityVersionControl({ repositoryId, repositoryName })
  const [activeTab, setActiveTab] = useState<
    'commits' | 'branches' | 'mergeRequests'
  >('commits')
  const [commitMessage, setCommitMessage] = useState('')
  const [newBranchName, setNewBranchName] = useState('')
  const [showNewBranchForm, setShowNewBranchForm] = useState(false)
  const [showCommitForm, setShowCommitForm] = useState(false)
  const [expandedCommits, setExpandedCommits] = useState<Set<string>>(new Set()) // Handle commit creation
    ...
    <div className={cn('flex flex-col h-full', className)}>
      ...
      >
        ...
        <div className="flex items-center space-x-2">
          ...
          >
            <Plus className="w-4 h-4" /> <span>New Branch</span>{' '}
          </button>{' '}
        </div>{' '}
      </GlassContainer>{' '}
      {/* Tab Navigation */}{' '}
                    ...
                    >
                      ...
                      <div className="flex items-center justify-between text-sm">
                        ...
                        <div className="flex items-center space-x-4 text-gray-400">
                          ...
                          <span>
                            {mr.approvals.length}/{mr.reviewers.length}{' '}
                            approvals
                          </span>{' '}
                        </div>{' '}
                        <div className="flex items-center space-x-2">
