...
interface LeaderboardCategoryManagerProps {
  ...
  onCategoriesChange: (_categories: LeaderboardCategory[]) => void; 
}
...
function LeaderboardCategoryManager({ categories, onCategoriesChange }: LeaderboardCategoryManagerProps) {
  const handleToggleCategory = (categoryId: string, enabled: boolean) => {
    ...
    onCategoriesChange(_updatedCategories);
  };
...
export function CommunityControls(_{ className }: CommunityControlsProps) {
  const [config, setConfig] = useState<CommunityConfig | null>(_null);
  const [featureFlags, setFeatureFlags] = useState<CommunityFeatureFlags | null>(_null);
  ...
  const loadCommunityConfig = async () => {
    ...
    try {
      ...
      setConfig(_mockConfig);
      setFeatureFlags(_mockFeatureFlags);
      setCategories(_mockConfig.leaderboards.categories);
    } catch (error) {
  ...
  const handleCategoriesChange = (_updatedCategories: LeaderboardCategory[]) => {
      ...
      <Card className="bg-white/10 backdrop-blur-md border-white/20 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FeatureFlagToggle
            ...
            onToggle={(_enabled) => handleFeatureFlagChange("'leaderboardsEnabled', enabled)}
            icon={Trophy}
          />
          <FeatureFlagToggle
            ...
            onToggle={(_enabled) => handleFeatureFlagChange("'statisticsEnabled', enabled)}
            icon={BarChart3}
          />
          <FeatureFlagToggle
            ...
            onToggle={(_enabled) => handleFeatureFlagChange("'realTimeUpdatesEnabled', enabled)}
            icon={Activity}
          />
          <FeatureFlagToggle
            ...
            onToggle={(_enabled) => handleFeatureFlagChange("'achievementsEnabled', enabled)}
            icon={Trophy}
          />
          <FeatureFlagToggle
            label="Badges System"
            ...
            onToggle={(_enabled) => handleFeatureFlagChange("'badgesEnabled', enabled)}
            icon={Shield}
          />
          <FeatureFlagToggle
            ...
            onToggle={(_enabled) => handleFeatureFlagChange("'streaksEnabled', enabled)}
            icon={Clock}
          />
        </div>
      </Card>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              onChange={(_e) => handleConfigChange("'performance', 'cacheTTL', parseInt(e.target.value) * 1000)}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              Update Interval (_seconds)
            </label>
            <input
              type="number"
              value={config.leaderboards.updateInterval / 1000}
              onChange={(_e) => handleConfigChange("'leaderboards', 'updateInterval', parseInt(e.target.value) * 1000)}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(_e) => handleConfigChange("'leaderboards', 'maxUsersPerPage', parseInt(e.target.value))}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(_e) => handleConfigChange("'performance', 'maxConcurrentUsers', parseInt(e.target.value))}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div>
            ...
            <input
              ...
              onChange={(_e) => handleConfigChange("'realTime', 'websocketUrl', e.target.value)}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              Fallback Polling Interval (_seconds)
            </label>
            <input
              type="number"
              value={config.realTime.fallbackPollingInterval / 1000}
              onChange={(_e) => handleConfigChange("'realTime', 'fallbackPollingInterval', parseInt(e.target.value) * 1000)}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(_e) => handleConfigChange("'realTime', 'maxReconnectAttempts', parseInt(e.target.value))}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-2">
              Heartbeat Interval (_seconds)
            </label>
            <input
              type="number"
              value={config.realTime.heartbeatInterval / 1000}
              onChange={(_e) => handleConfigChange("'realTime', 'heartbeatInterval', parseInt(e.target.value) * 1000)}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
        </div>
