...
export const PerformanceDashboard: React.FC = (_) => {
  const [stats, setStats] = useState<PerformanceStats | null>(_null);
  ...
  useEffect(() => {
    const fetchStats = (_) => {
      try {
        const currentStats = apiPerformanceMonitor.getStats(_timeWindow);
  ...
  const getHealthStatus = (_averageTime: number) => {
    if (_averageTime < 200) return { status: 'Excellent', color: 'text-green-400', bg: 'bg-green-500/20' };
    if (_averageTime < 500) return { status: 'Good', color: 'text-yellow-400', bg: 'bg-yellow-500/20' };
    if (_averageTime < 1000) return { status: 'Fair', color: 'text-orange-400', bg: 'bg-orange-500/20' };
  ...
  const health = getHealthStatus(_stats.averageResponseTime);
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-4">
          <select
            value={timeWindow}
            onChange={(_e) => setTimeWindow(_Number(e.target.value))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard
          ...
          value={stats.totalRequests.toLocaleString(_)}
          icon="📊"
        />
        <MetricCard
          title="Avg Response Time"
          value={`${Math.round(_stats.averageResponseTime)}ms`}
          icon="⚡"
          status={health.status}
        />
        <MetricCard
          title="Error Rate"
          value={`${(_stats.errorRate * 100).toFixed(1)}%`}
          icon="❌"
          status={stats.errorRate > 0.05 ? 'Poor' : stats.errorRate > 0.01 ? 'Fair' : 'Good'}
        />
        <MetricCard
          title="Slow Requests"
          value={`${(_stats.slowRequestRate * 100).toFixed(1)}%`}
          icon="🐌"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass border border-white/10 rounded-xl p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <span className="text-white font-mono">{Math.round(_stats.averageResponseTime)}ms</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-300">95th Percentile</span>
              <span className="text-white font-mono">{Math.round(_stats.p95ResponseTime)}ms</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-gray-300">99th Percentile</span>
              <span className="text-white font-mono">{Math.round(_stats.p99ResponseTime)}ms</span>
            </div>
          </div>
        </div>
        ...
        <div className="glass border border-white/10 rounded-xl p-6">
          ...
          <div className="space-y-2">
            {Object.entries(_stats.endpointStats)
              ...
              .map( ([endpoint, data]) => (
                <div key={endpoint} className="flex justify-between items-center py-2 border-b border-white/10 last:border-b-0">
                  ...
                  <div className="text-right">
                    <p className="text-white font-mono text-sm">{Math.round(_data.averageTime)}ms</p>
                    {data.errorRate > 0 && (
                      <p className="text-red-400 text-xs">{(_data.errorRate * 100).toFixed(1)}% errors</p>
                    )}
                  </div>
                </div>
              ))}
      ...
      <div className="glass border border-white/10 rounded-xl p-6">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {Object.entries(_stats.endpointStats).map( ([endpoint, data]) => {
                const endpointHealth = getHealthStatus(_data.averageTime);
                return (
                  <tr key={endpoint} className="border-b border-white/10 last:border-b-0">
                    ...
                    <td className="py-2 text-right text-white font-mono">{Math.round(_data.averageTime)}ms</td>
                    <td className="py-2 text-right text-gray-300">{(_data.errorRate * 100).toFixed(1)}%</td>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-end justify-between">
        <span className={`text-2xl font-bold ${getStatusColor(_status)}`}>{value}</span>
        {status && (
          <span className={`text-xs ${getStatusColor(_status)}`}>{status}</span>
        )}
      </div>
    </motion.div>
  );
