...
function LogDetailModal({ log, isOpen, onClose }: LogDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-gray-900 rounded-lg border border-white/20 w-full max-w-4xl max-h-[90vh] overflow-hidden">
        {/* Header */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {log.beforeState && (
              <Card className="bg-white/5 border-white/10 p-4">
                ...
                <div className="bg-black/20 rounded p-3 overflow-auto max-h-40">
                  <pre className="text-gray-300 text-xs">{formatJSON(_log.beforeState)}</pre>
                </div>
              </Card>
           )}
            ...
            {log.afterState && (
              <Card className="bg-white/5 border-white/10 p-4">
                ...
                <div className="bg-black/20 rounded p-3 overflow-auto max-h-40">
                  <pre className="text-gray-300 text-xs">{formatJSON(_log.afterState)}</pre>
                </div>
              </Card>
           )}

            {/* Metadata */}
            {log.metadata && Object.keys(_log.metadata).length > 0 && (
              <Card className="bg-white/5 border-white/10 p-4 lg:col-span-2">
                <h3 className="font-medium text-white mb-4">Metadata</h3>
                <div className="bg-black/20 rounded p-3 overflow-auto max-h-40">
                  <pre className="text-gray-300 text-xs">{formatJSON(_log.metadata)}</pre>
                </div>
              </Card>
           )}
          </div>
  ...
  const loadLogs = async () => {
    ...
    try {
      const params = {
        ...
        startDate: filters.startDate ? new Date(_filters.startDate) : undefined 
        endDate: filters.endDate ? new Date(_filters.endDate) : undefined 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select
              ...
              onChange={(_e) => setFilters(prev => ({ ...prev, category: e.target.value }))}
          ...
          <div>
            ...
            <select
              ...
              onChange={(_e) => setFilters(prev => ({ ...prev, success: e.target.value }))}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
          <div>
            ...
            <input
              ...
              onChange={(_e) => setFilters(prev => ({ ...prev, startDate: e.target.value }))}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
          <div>
            ...
            <input
              ...
              onChange={(_e) => setFilters(prev => ({ ...prev, endDate: e.target.value }))}
              className="w-full px-3 py-2 bg-white/10 border border-white/20 rounded-lg text-white focus:outline-none focus:border-blue-400"
            />
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="px-6 py-4">
                        <span className={cn(
                          ...
                          getSeverityColor(_log.severity)
                       )}>
                          {log.severity}
                        </span>
                      </td>
