...
const EnhancedAIAssistantComponent: React.FC<EnhancedAIAssistantProps> = ({
  ...
  const sendMessage = async (message: string, type: string = "question") => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setMessages((prev: unknown) => [...prev, assistantMessage]);
      ...
      setMessages((prev: unknown) => [...prev, errorMessage]);
  ...
  const extractConceptFromMessage = (message: string): string | null => {
    ...
    const foundConcept = conceptKeywords.find((keyword: unknown) =>
    lowerMessage.includes(keyword),
  );
  return foundConcept || null;
};
// Enhanced sendMessage with callback usage
const sendMessageWithCallbacks = async (
  message: string,
...
const handleQuickAction = (action: (typeof quickActions)[0]) => {
  ...
  switch (action.type) {
    case "code-review":
    message = currentCode
    ? `Please review this Solidity code: ${currentCode}`
    : "Please explain how to write good Solidity code";
    break;
    case ",
    debug":
...
const exportChat = () => {
  const chatData = messages.map((msg: unknown) => ({
    type: msg.type,
    content: msg.content,
...
return (
  ...
  {" "}
  <Card
  className={`bg-white/10 backdrop-blur-md border border-white/20 transition-all duration-300 ${isExpanded ? "fixed inset-4 z-50" : "h-96"}`}>{" "}
  {/* Header */}{" "}
  ...
  variant="outline"
  size="sm"
  className="border-white/30">{isSpeaking ? (
  ...
  variant="outline"
  size="sm"
  className="border-white/30">{isExpanded ? (
  ...
  {quickActions.map((action: unknown) => (
    ...
    disabled={isLoading}><action.icon className="w-3 h-3 mr-1" /> {action.label}{" "}
    </Button>
  ))}{" "}
  </div>{" "}
  </div>{" "}
  ...
  {messages.length === 0 && (
    ...
    I can help with code review, debugging, concepts, and more.
    </p>{" "}
    </div>
  )}{" "}
  {messages.map((message: unknown) => (
    ...
    className={`max-w-[80%] p-3 rounded-lg ${message.type === "user" ? "bg-blue-600 text-white" : "bg-gray-700 text-gray-100"}`}>{" "}
    ...
    {message.type === "assistant" && (
      ...
      variant="ghost"
      size="sm"
      className="h-6 w-6 p-0"><Copy className="w-3 h-3" />{" "}
      </Button>{" "}
  ...
  {isLoading && (
    ...
    initial={{ opacity: 0 }}
    animate={{ opacity: 1 }}
    className="flex justify-start"><div className="bg-gray-700 text-gray-100 p-3 rounded-lg">
    {" "}
    <div className="flex items-center space-x-2">
    {" "}
  ...
  e.key === "Enter" && sendMessageWithCallbacks(inputMessage)
}
placeholder={
  isListening
  ? "Listening..."
...
className="bg-blue-600 hover:bg-blue-700"><Send className="w-4 h-4" />{" "}
</Button>{" "}
</div>{" "}
</div>{" "}
</Card>{" "}
...
{showSettings && (
  ...
  animate={{ opacity: 1, y: 0 }}
  exit={{ opacity: 0, y: -20 }}
  className="absolute top-16 right-0 z-50 w-64 p-4 bg-white dark:bg-gray-800 rounded-lg shadow-lg border border-gray-200 dark:border-gray-700"><h4 className="font-semibold mb-4">AI Assistant Settings</h4>{" "}
  ...
  variant="outline"
  size="sm"
  className="w-full justify-start"><Download className="w-4 h-4 mr-2" /> Export Chat{" "}
  </Button>{" "}
  <Button
  onClick={clearChat}
  variant="outline"
  size="sm"
  className="w-full justify-start"><RotateCcw className="w-4 h-4 mr-2" /> Clear Chat{" "}
  </Button>{" "}
  </div>{" "}
  </motion.div>
)}{" "}
...
export const EnhancedAIAssistant = withAsyncErrorBoundary(
  EnhancedAIAssistantComponent,
  {
