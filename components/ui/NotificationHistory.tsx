...
export function NotificationHistoryModal( { isOpen, onClose }: NotificationHistoryModalProps) {
  const { history, clearHistory } = useNotifications(_);
  ...
  const filteredAndSortedHistory = useMemo(() => {
    ...
    switch (_sortBy) {
      ...
      case 'oldest':
        ...
        break;
      case 'type':
        filtered.sort( (a, b) => a.type.localeCompare(_b.type));
        break;
    }
  ...
  const formatTimestamp = (_timestamp: number) => {
    ...
    const diffMins = Math.floor(_diffMs / 60000);
    const diffHours = Math.floor(_diffMs / 3600000);
    const diffDays = Math.floor(_diffMs / 86400000);

    if (_diffMins < 1) return 'Just now';
    if (_diffMins < 60) return `${diffMins}m ago`;
  ...
  const getIcon = (_type: NotificationType) => {
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center space-x-3">
                ...
                <span className="text-sm text-gray-400">
                  (_{filteredAndSortedHistory.length} of {history.length})
                </span>
              </div>
            ...
            <div className="space-y-4 mb-6">
              ...
              <div className="flex flex-wrap gap-2">
                {notificationTypes.map((type) => (
                  <button
                    key={type.type}
                    onClick={(_) => setSelectedType(_type.type)}
                    className={cn(
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex space-x-2">
                  {[
                    ...
                    <button
                      key={option.value}
                      onClick={(_) => setSortBy(_option.value as any)}
                      className={cn(
...
interface HistoryItemProps {
  notification: Notification;
  formatTimestamp: (_timestamp: number) => string;
  getIcon: (_type: NotificationType) => React.ReactNode;
}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2 text-xs text-gray-400">
              ...
              <span>{formatTimestamp(_notification.timestamp)}</span>
            </div>
          </div>
