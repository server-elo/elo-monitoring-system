...
export function LoadingSpinner({
  ...
  const renderSpinner = () => {
    switch (variant) {
      ...
      return (
        ...
        className={cn(sizeClass, colorClass, className)}><RotateCw className="w-full h-full" />
        </motion.div>
      );
      default:
      return (
        ...
        className={cn(sizeClass, colorClass, className)}><Loader2 className="w-full h-full" />
        </motion.div>
      );
    }
  };
  ...
  const containerClass = cn("flex items-center", {
...
export function ButtonLoadingSpinner({
  className,
  ...props
}: Omit<LoadingSpinnerProps, "size" | "variant">): void {
  return (
    <LoadingSpinner
    {...props}
    size="sm"
